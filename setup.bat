...
powershell -NoProfile -Command "$ws = New-Object -ComObject WScript.Shell; $s = $ws.CreateShortcut('%STARTUP_DIR%\Rat Race.lnk'); $s.TargetPath = '%JOBFINDER_EXE%'; $s.WorkingDirectory = '%ROOT%\jobfinder'; $s.Description = 'Rat Race Job Finder'; $s.Save()"
...
powershell -NoProfile -Command "$ws = New-Object -ComObject WScript.Shell; $desktop = $ws.SpecialFolders('Desktop'); $s = $ws.CreateShortcut(\"$desktop\Job Results.lnk\"); $s.TargetPath = '%RESULTS_DIR%'; $s.Description = 'Rat Race Job Results'; $s.Save()"
...
powershell -NoProfile -Command "$ws = New-Object -ComObject WScript.Shell; $desktop = $ws.SpecialFolders('Desktop'); $s = $ws.CreateShortcut(\"$desktop\Rat Race Login.lnk\"); $s.TargetPath = '%LOGIN_EXE%'; $s.WorkingDirectory = '%ROOT%\login'; $s.Description = 'Rat Race Login Setup'; $s.Save()"
